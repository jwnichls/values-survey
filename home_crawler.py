...
def save_tweets(uid, tweets):
    ...
    max_id = None
    total = 0
    for authoruid, authorsname, tid, created, message, retweet, mention, hashtag, url, jstring in tweets:
        cur.execute("""insert ignore into home_tweet
                            values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""", 
                            (uid, authoruid, authorsname, tid, created, message, retweet, mention, hashtag, url, jstring))
    conn.commit()
    cur.execute("select min(tid), count(tid) from home_tweet where uid = %s", (uid))
    row = cur.fetchone()
    max_id, total = row
    updated = int(time.time())
    if len(tweets) < 190 or total > MAX_TWEETS:
        completed = True
    cur.execute("replace into home_status values (%s, %s, %s, %s)", (uid, total, max_id, completed))
    conn.commit()
    logging.info("Saved tweets for %s. Now have %s tweets for %s" % (uid, total, uid))
    return completed, max_id
        
    
def timeline_portion(twitter, max_id):
    ...
    for t in tl:
        ...
        authoruid = t['user']['id']
        authorsname = t['user']['screen_name']
        ...
        tweets.append((authoruid, authorsname, tid, created, message, retweet, mention, hashtag, url, jstring))
        
    return tweets

def home_timeline(twitter, uid, max_id=None):
    error = 0
    tweets = []
    ...
    while error < 10:
        try:
            time.sleep(INTERVAL)
            tweets = timeline_portion(twitter, max_id)
...
def crawl_home(log_file):
    ...
    while True:
        conn.commit()
        cur.execute("select u.uid, u.token, u.secret, hs.tid from users u left join home_status hs on u.uid = hs.uid where (hs.completed is null or hs.completed = 0) order by u.created_at asc limit 100")
        ...
        while not q.empty():
            ...
            home_timeline(t, uid, tid)

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(
            description='Crawl home timeline for people')
    parser.add_argument('log_file', help='path to the log file')
    
    args = parser.parse_args()
    crawl_home(args.log_file)